...
export default function Women({ onAddToCart, onOpenAuth }: WomenProps) {
  ...
  return (
    <div className="min-h-screen pt-16">
      ...
      <div className="py-16 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              { name: 'Dresses', image: '/1G5A2179.jpg' },
              { name: 'T-Shirts', image: '/1G5A2160.jpg' },
              { name: 'Accessories', image: '/1G5A2109 copy.jpg' }
            ].map((category) => (
              <div 
                key={category.name}
              ...
              >
                ...
                {/* Overlay Gradients with Purple Tint */}
                <div className="absolute inset-0 bg-gradient-to-t from-purple-900/70 via-purple-500/20 to-transparent z-10"></div>
                <div className="absolute inset-0 bg-purple-500/20 group-hover:bg-purple-500/10 transition-all duration-300 z-10"></div>
                
                {/* Category Content */}
                <div className="absolute bottom-6 left-6 z-20">
                  <h3 className="text-2xl font-bold text-white mb-2 drop-shadow-lg">{category.name}</h3>
                  <p className="text-purple-100 drop-shadow-md">Explore Collection</p>
                </div>
                
                {/* Hover Icon */}
                <div className="absolute top-4 right-4 z-20 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                  <div className="w-12 h-12 bg-white/20 backdrop-blur-sm rounded-full flex items-center justify-center border border-purple-300/50">
                    <ShoppingBag className="w-6 h-6 text-white" />
                  </div>
                </div>
                
                {/* Subtle border animation with purple theme */}
                <div className="absolute inset-0 rounded-2xl border-2 border-transparent group-hover:border-purple-300/40 transition-all duration-300 z-10"></div>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Products Section */}
      <div id="products-section" className="py-20 bg-gradient-to-b from-purple-50 to-white relative">
        {/* Subtle Background Pattern */}
        <div className="absolute inset-0 opacity-5">
          <div className="absolute inset-0" style={{
            backgroundImage: `url("data:image/svg+xml,%3csvg width='60' height='60' viewBox='0 0 60 60' xmlns='http://www.w3.org/2000/svg'%3e%3cg fill='none' fill-rule='evenodd'%3e%3cg fill='%23800080' fill-opacity='1'%3e%3ccircle cx='30' cy='30' r='1'/%3e%3c/g%3e%3c/g%3e%3c/svg%3e")`
          }}></div>
        </div>
